...
export default function NavRail({ activeIndex }: { activeIndex: number }) {
  return (
    <div className="absolute left-2 top-5 bottom-8 w-9 flex flex-col items-center z-40 pointer-events-none">
      ...
      <div className="h-9 w-9 shrink-0 overflow-hidden rounded-full ring-[0.5px] ring-white/30 bg-[#0f0f13]">
      ...
      <div className="relative mt-[52px] flex flex-1 flex-col items-center">
        ...
        <div className="absolute top-[8px] bottom-[8px] w-[0.5px] bg-white/[0.12]" />
          ...
          {[0, 1, 2, 3, 4].map((i) => {
            ...
            return (
              <div key={i} className="flex h-5 w-5 items-center justify-center">
                {isActive ? (
                  <span className="flex h-[18px] w-[18px] items-center justify-center rounded-full bg-star/15 ring-[0.5px] ring-star/40">
                    <span className="block h-[7px] w-[7px] rounded-full bg-star shadow-[0_0_8px_var(--color-star)]" />
                  </span>
                ) : (
                  <span className="block h-[3.5px] w-[3.5px] rounded-full bg-white/[0.22]" />
                )}
              </div>
            );
          })}
